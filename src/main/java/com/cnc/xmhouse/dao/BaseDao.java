...
public class BaseDao<T> {
    ...
    public List queryNoneType(String hql,Map<String,Object> hs){
        Session session = sessionFactory.getCurrentSession();
        Query query = session.createQuery(hql);
        query.setProperties(hs);
        return query.list();
    ...
    public List queryNoneType(String hql){
        Session session = sessionFactory.getCurrentSession();
        Query query = session.createQuery(hql);
        return query.list();
    }
    ...
    public List<T> queryWithStartLimit(String hql, Map<String, Object> hs,Integer start,Integer limit) {
        Session session = sessionFactory.getCurrentSession();
        Query query = session.createQuery(hql);
        setQueryParam(hs, query);
    ...
    public List<T> queryWithStartLimit(String hql, Integer start,Integer limit,Object... params) {
        Session session = sessionFactory.getCurrentSession();
        Query query = session.createQuery(hql);
    ...
    public List querySqlNoneType(String sql) {
        Session session = sessionFactory.getCurrentSession();
    ...
    public List<T> querySqlWithStartLimit(String hql,Integer start,Integer limit){
        Session session = sessionFactory.getCurrentSession();
        Query query = session.createSQLQuery(hql);
        query.setFirstResult(start*limit);
        query.setMaxResults(limit);
    ...
    public Pair<Integer,List<T>> countQuerySqlLimit(String sql) {
        Session session = sessionFactory.getCurrentSession();
        Query query = session.createSQLQuery(sql);
        List<T> list = query.list();
        int count=0;
...
//            }
    ...
    public Pair<Long,List<T>> critetiriaPaging(Class<T> clazz,List<SimpleExpression>ress,int page,int perpage) {
        Criteria criteria = sessionFactory.getCurrentSession().createCriteria(clazz);
        ress.stream().forEach(res->criteria.add(res));
        criteria.setFirstResult(page*perpage).setMaxResults(perpage);
        List list = criteria.list();
        Criteria countCriteria = sessionFactory.getCurrentSession().createCriteria(clazz);
        ress.stream().forEach(res->countCriteria.add(res));
...
//        return session.load(clazz, id);
    ...
    public T queryGetOne(String hql, Object... params) {
        Session session = sessionFactory.getCurrentSession();
        Query query = session.createQuery(hql);
        setParam(query, params);
